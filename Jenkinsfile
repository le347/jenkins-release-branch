      ...
      steps {
        container('git') {
          withCredentials([usernamePassword(credentialsId: 'git-credentials-acm', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
            ...
            sh "git clone https://${GIT_USERNAME}:${GIT_PASSWORD}@${env.BASTION_IP}:/${env.GITHUB_ORGANIZATION}/${env.SERVICE}"
            sh "version=`cat ${env.SERVICE}/version`"
            sh "cd ${env.SERVICE}/ && git checkout -b release/`cat version`"
            sh "cd ${env.SERVICE}/ && git push --set-upstream https://${GIT_USERNAME}:${GIT_PASSWORD}@${env.BASTION_IP}:/${env.GITHUB_ORGANIZATION}/${env.SERVICE} release/`cat version`"
            ...
            sh "cd ${env.SERVICE}/ && git push https://${GIT_USERNAME}:${GIT_PASSWORD}@${env.BASTION_IP}:/${env.GITHUB_ORGANIZATION}/${env.SERVICE}"
          } 
        }
      }
    }
